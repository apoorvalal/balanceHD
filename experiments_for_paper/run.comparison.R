run.comparison = function(XYW) {
  ...
  tau.aipw = ipw.ate(X, Y, W,
    target.pop = 1, prop.weighted.fit = FALSE, targeting.method = "AIPW",
    fit.method = "elnet", prop.method = "elnet", alpha.fit = 0.9, alpha.prop = 0.5
  )
  tau.ipw.weighted = ipw.ate(X, Y, W,
    target.pop = 1, prop.weighted.fit = TRUE, targeting.method = "AIPW",
    fit.method = "elnet", prop.method = "elnet", alpha.fit = 0.9, alpha.prop = 0.5
  )
  tau.tmle = ipw.ate(X, Y, W,
    prop.weighted.fit = FALSE, targeting.method = "TMLE",

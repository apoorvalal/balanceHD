...
ipw.ate = function(X, Y, W, target.pop = c(0, 1), eps.threshold = 1 / 20,
  ...
  if (prop.method == "elnet") {
    ...
    if (!requireNamespace("grf", quietly = TRUE)) {
      stop("Package grf is needed for this function to work. Please install it.", call. = FALSE)
    }
    prop.fit = grf::probability_forest(X, factor(W))
    prop.raw = predict(prop.fit)[[1]][, 2]
  } else {
  ...
  prop = prop.raw
  if (0 %in% target.pop) prop = pmax(prop, eps.threshold)
  if (1 %in% target.pop) prop = pmin(prop, 1 - eps.threshold)
  ...
  if (targeting.method == "AIPW") {
    mu.residual = c(
      sum(prop.weights[W == 0] * residuals[W == 0]),
    ...
    # targeted MLE
    eps.tmle.0 = lm(B ~ A + 0,
      data = data.frame(A = prop.weights[W == 0] * sum(W == 0), B = residuals[W == 0]))
    eps.tmle.1 = lm(B ~ A + 0,
      data = data.frame(A = prop.weights[W == 1] * sum(W == 1), B = residuals[W == 1]))
